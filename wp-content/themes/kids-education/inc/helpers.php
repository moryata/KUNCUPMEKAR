		...
		if ( true == $options['pagination_enable'] ) {
			...
			elseif ( $pagination == 'numeric' ) :
				the_posts_pagination( array(
				    'mid_size' => 2,
				    'prev_text' => esc_html__( 'Previous', 'kids-education' ),
				    'next_text' => esc_html__( 'Next', 'kids-education' ),
				) );
			endif;
		}
	}
...
if ( ! function_exists( 'kids_education_post_pagination' ) ) :
	...
	function kids_education_post_pagination() {
		...
		if( $options['post_navigation_enable'] ){
			...
			$args = array( 
				'prev_text'          => esc_html__( 'Prev Post', 'kids-education' ),
	            'next_text'          => esc_html__( 'Next Post', 'kids-education' ),
	            'in_same_term'       => true,
	            'screen_reader_text' => esc_html__( 'Continue Reading', 'kids-education' ),
	        );

	        if( 'post_title' == $options['post_navigation_type'] ){
	        	$args = array(
		        	'prev_text'                  => esc_html__( '%title', 'kids-education' ),
	            	'next_text'                  => esc_html__( '%title', 'kids-education' ),
	            	'in_same_term'               => true,
	            	'screen_reader_text' => esc_html__( 'Continue Reading', 'kids-education' ),
	            );
	        }
...
function kids_education_get_author_profile(){
    ...
    $author_url = !empty( $url ) ? get_the_author_meta( 'user_url' ) : '#' ;
    ?>
...
function kids_education_get_related_posts(){
    ...
    ?>
          ...
          <h2 class="related-post-title"><?php esc_html_e( 'Related posts', 'kids-education' ); ?></h2>
          ...
          <article id="post-<?php the_ID(); ?>" class="column-wrapper <?php post_class(); ?>">
            <div class="blog-post-wrap">
              ...
              <header class="entry-header">
                ...
                <p class="entry-meta">
                  <span class="posted-on"><span class="screen-reader-text"><?php esc_html_e( 'Posted on', 'kids-education'); ?></span>
              ...
              <div class="entry-content">
                ...
                <a href="<?php echo esc_url( $post_url ); ?>" class="read-more"><?php esc_html_e( 'Read More','kids-education'); ?><i class="fa fa-angle-right"></i></a>
              </div><!-- .entry-content -->
            </div><!-- .blog-post-wrap -->
          </article><!-- #post -->
          <?php endforeach; ?>
