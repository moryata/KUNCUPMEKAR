...
function kids_education_posted_on() {
	...
	$posted_on = '<span  class="screen-reader-text">'. esc_html__( 'Posted on', 'kids-education' ) .'</span>
	...
	if( function_exists( 'tp_education_like_button' ) ){
		...
		$like_string = '<span class="screen-reader-text">' . esc_html__( 'Likes', 'kids-education' ) . '</span>' . $like_string;
		echo wp_kses_post( $like_string );
	}

    $no_of_comment = get_comments_number(); // get no of comments in a post

    $comment_number = sprintf( '<span class="screen-reader-text">'. esc_html__( 'Comments', 'kids-education' ) .'</span>
...
function kids_education_tags_list() {
	...
	if( !empty( $tags_list ) ){
		echo '<div class="tag-list">
		  		<span class="screen-reader-text">'. esc_html__( 'Tagged', 'kids-education') .' </span>';

		printf( '<span class="tags-links"> ' . esc_html__( 'Tags: %1$s', 'kids-education' ) . '</span>', $tags_list );
		echo '</div>';
	}
}
endif;
