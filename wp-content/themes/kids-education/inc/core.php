...
function kids_education_slider_image_instruction( $content, $post_id ) {
  ...
  if ( in_array( get_post_type( $post_id ), $allowed ) ) {
    return $content .= '<p><b>' . esc_html__( 'Note', 'kids-education' ) . ':</b>' . esc_html__( ' The recommended size for image is 1350px by 550px while using it for slider', 'kids-education' ) . '</p>';
  } elseif ( 'jetpack-testimonial' == get_post_type( $post_id ) ) {
    return $content .= '<p><b>' . esc_html__( 'Note', 'kids-education' ) . ':</b>' . esc_html__( ' The recommended size for image is 340px by 340px while using it for testimonial', 'kids-education' ) . '</p>';
  }
   return $content;
}
